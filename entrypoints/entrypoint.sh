...
#cd /opt/cryosparc_master/entrypoints/ && ../bin/cryosparcm start && cd /
##cd /opt/cryosparc_master/entrypoints/ && ../bin/cryosparcm restart && cd /
#cd /opt/cryosparc_master/entrypoints/ && ../bin/cryosparcm createuser --email "a@a.com" --password "a" --username "myuser" --firstname "John" --lastname "Doe" && cd /
#cd /opt/cryosparc_worker/bin/ && ./cryosparcw connect --master "127.0.0.1" --worker "127.0.0.1" --port 8080 && cd /
#

# Source: https://github.com/slaclab/cryosparc-docker/blob/master/cryosparc-server.sh

# ensure we have a cryosparc directory under home
export CRYOSPARC_DATADIR=/opt
...
#  exit 127
#fi
CRYOSPARC_BASE_PORT=${CRYOSPARC_BASE_PORT:-"8080"}
...
#${CRYOSPARC_MASTER_DIR}/cryosparcm cli 'get_scheduler_targets()' | python -c "import sys, ast, json; print( json.dumps(ast.literal_eval(sys.stdin.readline())) )" | jq '.[].name' | sed 's:"::g' | xargs -n1 -I \{\} ${CRYOSPARC_MASTER_DIR}/cryosparcm cli 'remove_scheduler_target_node("'{}'")'
## add slurm lanes
#for i in `ls -1 /app/slurm/`; do
#  cd /app/slurm/$i
#  ${CRYOSPARC_MASTER_DIR}/cryosparcm cluster connect
#done
#cd ${CRYOSPARC_MASTER_DIR}
#
...
${CRYOSPARC_WORKER_DIR}/bin/cryosparcw connect --worker ${CRYOSPARC_MASTER_HOSTNAME} --master ${CRYOSPARC_MASTER_HOSTNAME} --ssdpath $TMPDIR/
